...
const GITHUB_REPO_URL = 'https://github.com/jhotadhari/react-native-brouter';
...
function getNpmTag(version: string): string | undefined {
	...
	return parsed.prerelease[0] as string; // "alpha", "beta", "rc"
}
	...
	if (!token) {
		fatalError(
			'GITHUB_TOKEN environment variable is required to create a GitHub release. ' +
				'Run `export GITHUB_TOKEN=$(gh auth token)` or generate one at https://github.com/settings/tokens (scope: repo).'
		);
	}
